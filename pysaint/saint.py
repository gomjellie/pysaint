...
class Saint:
    ...
    def _select_related_major(self, course_name):
        """
        연계전공에서 과목을 선택하고 검색 버튼을 누른다.
        선행작업:
        CourseParser.select_course_section('연계전공')
        :param course_name:
        :return:
        """

        course_id = get_related_major_key(self.soup_jar['연계전공'])
        course_skey = get_related_major_skey(self.soup_jar['연계전공'], course_name)
        search_id = get_search_id(self.soup_jar['연계전공'])
        dt_combo_with_search_button_press \
            = sap_event_queue.combo_select_with_button_press(course_id, course_skey, search_id, self.sap_wd_secure_id)
        on_selective_liberal_course = self.sess.post(ECC_URL + self.action, data=dt_combo_with_search_button_press)
        self.soup_jar['related_major_search'] = BeautifulSoup(on_selective_liberal_course.text, 'lxml')

        return parse_subjects(self.soup_jar['related_major_search'])
    
    def _select_fusion_major(self, course_name):
        """
        융합전공에서 과목을 선택하고 검색 버튼을 누른다.
        선행작업:
        CourseParser.select_course_section('융합전공')
        :param course_name:
        :return:
        """

        course_id = get_fusion_major_key(self.soup_jar['융합전공'])
        course_skey = get_fusion_major_skey(self.soup_jar['융합전공'], course_name)
        search_id = get_search_id(self.soup_jar['융합전공'])
        dt_combo_with_search_button_press \
            = sap_event_queue.combo_select_with_button_press(course_id, course_skey, search_id, self.sap_wd_secure_id)
        on_selective_liberal_course = self.sess.post(ECC_URL + self.action, data=dt_combo_with_search_button_press)
        self.soup_jar['fusion_major_search'] = BeautifulSoup(on_selective_liberal_course.text, 'lxml')
    ...
    def select_on_related_major(self, course_name):
        """
        교양선택에서 과목을 선택해서 검색 정보를 얻는다
        선행작업:
        CourseParser.select_course_section('연계전공')
        ...
        :return:
        """
        return self._select_related_major(course_name)

    def select_on_fusion_major(self, course_name):
        """
        융합전공에서 과목을 선택해서 검색 정보를 얻는다
        선행작업:
        CourseParser.select_course_section('융합전공')
        :param course_name:
        :return:
    ...
    def get_related_major_map(self):
        """
        :TODO :
        CourseParser.select_course_section('연계전공') 을 처음 실행해서 얻게된
        self.soup_jar['연계전공'] 에 과목정보가 있는경우가 있고 없는경우가 있다.
        과목정보가 변경되면 과목정보가 생기고 그대로면 안생기는듯
        교양선택 맵을 얻는다.
        선행되야하는 작업:
        :return:
        ...
        self.select_course_section('연계전공')
        courses = get_related_major_courses(self.soup_jar['연계전공'])

        return courses

    def get_fusion_major_map(self):
        """
        :TODO :
        CourseParser.select_course_section('융합전공') 을 처음 실행해서 얻게된
        self.soup_jar['융합전공'] 에 과목정보가 있는경우가 있고 없는경우가 있다.
        과목정보가 변경되면 과목정보가 생기고 그대로면 안생기는듯
        교양선택 맵을 얻는다.
        선행되야하는 작업:
        :return:
        ...
        self.select_course_section('융합전공')
